...
class Person(object):
    def age_majority(self, person_name, person_gender, person_age):
        if person_gender == "M" and float(person_age) >= 18:
            return person_name + " has come of age!"

        elif person_gender == "M" and float(person_age) < 18:
            return person_name + " hasn't come of age. Males don't come of age until they are 18!"

        if person_gender == "F" and float(person_age) >= 21:
            return person_name + " has come of age!"

        elif person_gender == "F" and float(person_age) < 21:
            return person_name + " hasn't come of age. Females don't come of age until they are 21!"

daemon = Pyro5.api.Daemon()  # make a Pyro daemon
ns = Pyro5.api.locate_ns()  # find the name server
uri = daemon.register(Person)  # register Person as a Pyro object
